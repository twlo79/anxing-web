...
```
draft ──送出──┬─ 總額 < 3000 ──────────────> approved
              └─ 總額 ≥ 3000 ──> pending_manager
                                      │
                        主管核可 ──────┴──> pending_ceo ──CEO核可──> approved
                        駁回 ──────────────> rejected
                                             pending_ceo 駁回 ────> rejected

rejected ──申請人修改後重送──> 回到送出判斷
approved ──填入採購日──────> 產生支出（不可逆）
```

**低於 3000 兩關全免**，直接進 `approved`。門檻看**整張單總額**，不是單一項目 —— 看項目會留下拆單規避的空間（5 項各 2,900 總共 14,500 全部免核）。

**核可不可逆。** 進 `approved` 之後不能編輯、不能刪除，唯一還能改的是 `purchased_on`（採購日）。

**駁回必填原因。** 單子退回 `rejected`，申請人可以改，改完重新送審。沒有駁回機制的話，一張金額打錯的單會卡死 —— 不能編輯、不能刪除、又不該核可。
...
| | 請款單 | 支出 |
|---|---|---|
| **housekeeper**（管家） | 建立；編輯／刪除**自己**的 `draft`、`rejected`；檢視自己送的全部 | **完全看不到** |
| **manager**（主管） | 檢視全部；**第一關核可／駁回**；自己也能送單 | 檢視、新增、編輯、刪除 |
| **super_admin** | 全部；**第二關核可／駁回** | 全部 |

核可鏈綁**角色**不綁人：第一關任一 `manager`，第二關任一 `super_admin`。人員異動只要改角色，不用改程式。

⚠️ **副作用：David 是 super_admin，所以也能核第二關。** 如果要讓 CEO 專責，得另外開一個 `ceo` 角色 —— 但那會動到現有 RLS 的 `current_role_of()`，全表都要重新檢查。目前不做。

⚠️ **自己送的單不能自己核。** 主管送的單第一關要由**其他**主管核，或直接跳給 super_admin。這條寫在 policy 裡。
