...
> 2026-09-05 從 `README.md` 中段抽出來。
>
> **為什麼分家**：那 2000 行原本是**舊版 README 整份被貼進主文件裡**，
> 夾在第十一章與第十二章中間，27 個章節一個編號都沒有。
> 結果是「角色與權限」「各表說明」「Migration 索引」在同一份文件裡各出現兩次，
> 而兩份講的已經不一樣了 —— 而看的人不知道該信哪一份。
>
> **分工**：`README.md` 講**規則與為什麼**（流程、踩過的坑、介面規矩）；
> 這一份是**查表用的**（有哪些表、哪個欄位是什麼、哪支 migration 做了什麼）。
> 要知道「為什麼這樣設計」看 README，要知道「這一欄叫什麼」看這裡。
...
| 項目 | 說明 |
|---|---|
| **「按掉」會讓手動加入的入口消失** | 房務例外分頁的「手動加入」按鈕**只長在「沒進系統的」清單的每一列上**。把那幾筆都按掉之後那一區變 0 筆，按鈕跟著不見 —— 表單還在程式裡，只是進不去了（2026-09-03 使用者卡在這裡）。<br>★ 2026-09-03 把「間數」欄補到排班表的房源格（新增與就地編輯都有），所以現在不靠那個入口也改得到。但「按掉會收走工具」這件事本身還沒修。 |
...
| **收款權限只擋在畫面上** | 2026-09-02 把契約收租、短租訂單收款、押金收款三顆按鈕收給會計與總經理（`src/lib/collect-perm.ts`），但 **RLS 一個字都沒動** —— `orders` / `deposits` 的 update policy 仍然放行主管與管家。<br>★★ 所以匯入、API、之後新寫的任何一段程式都繞得過去。前端比 RLS 窄是「按鈕點不下去」，不是「改不了」。<br>★ 同一排的「退回未收」與「收款日」也沒擋 —— 主管仍可以把已收退回未收、或把收款日改掉。使用者這次只點名「收款」，要不要一起收得問過他 |
| **★★★ 房務支出:工單與支出會失去對應** | `hk_job_key`（日期│房源│類型）是唯一判斷「產生過了沒」的依據，而寫入是 `upsert ＋ ignoreDuplicates`。三種情況會讓帳偏掉，**而且都不會叫**:<br>① **手動在支出頁寫的房務支出沒有 key** → 系統看不見它，按產生會再建一筆，同一次清潔算兩次<br>② **改房源或日期** → key 換掉 → 新增一筆、舊的留在帳上（重複計）<br>③ **刪掉工單** → 支出留在帳上變孤兒<br>★ 三者都會讓總額偏掉，而每一筆單獨看都很正常。2026-09-03 的 migration_214／215 就是在手動補這個洞。<br>★★ 解法是預覽表要列出「已產生但對不上」與「孤兒」，讓人決定 —— 不自動改（已經對過的帳不該無聲變動）。 |
| **房務支出:改了設定不會回頭改已產生的** | 改間數、金額、人事費之後重按「產生本月支出」——**什麼都不會發生**（`ignoreDuplicates`）。那個設計是對的（已對過的帳不該無聲變動），但代價是修正完全傳不過去，只能寫 migration。<br>★ 2026-09-03 的 `amount_override` 是第一步:讓「人調過的」有地方存。還缺的是**已產生之後也能調**。 |
| **房務支出:「N 份工算不出錢」只講兩種原因** | 實際上一份工掉出去有五條路，畫面只講「沒有房源」「沒設單價」兩條。另外三條完全不提:代碼對不到主檔、對到了但沒指向 ERP 房源、**執行的人不是按間數計酬**。<br>★★ 最後那條最危險 —— 劉姐是時薪制（`count_mode='hours'`），她掃的每一間**從來不會產生清潔費**，而排班表上她跟別人長得一模一樣。這不是 bug 是設計（她領時薪不是件數），但畫面完全不提，看起來像漏了。<br>★ 2026-09-03 使用者靠肉眼比對排班表才發現「有的沒進去」，然後花了兩小時逐條查。 |
| **房務:改了項目名稱關掉就沒了** | 產生預覽的「項目」欄可以改名，但只活在對話框打開的期間 —— 沒有存回工單。當初的理由是「它只用一次」，而使用者第一次用就問「可以暫存嗎」（2026-09-03）。 |
| **房務例外:「同月清掃 3 次以上」對公區與整棟是誤報** | 4B2、開封整棟、復興、時兆公區每個月都會被報 —— 那不是重複建立，是它們真的一個月被掃三五次。<br>★ 假警報看久了，真的那一筆會被當成雜訊跳過（CLAUDE.md:「誤報 46 筆是整份清單再也沒有人看」）。該排除 `ptype='common_area'` 與整棟類。 |
...
| 171–199 | `archive/migrations-171-199/` |
| **200 之後** | **`supabase/migrations/`（還在演進中，目前到 220）** |

**200 之後這一批做了什麼**（2026-09 更新）：
...
| 204 | 免稅設定 |
| 205–208 | 房務代碼合併、成本表、單價連結、`hk_labor_cost` 的 RLS（206 漏了 policy，208 補） |
| 209 | 轉帳部分沖銷 |
| 210 / 211 | 房務支出的物業歸屬、`hk_day.rooms_override`（migration_60 寫了但沒生效） |
| 212 / 213 | 暫付加用途；零用金 30,000 從支出搬到暫付 |
| 215 / 216 | 工單金額覆寫（`amount_override`）、拆帳表（`hk_work_split`） |
| 217 | 營業稅：`tax_period` ＋ `tax_invoice` ＋ RLS |
| **218** | **請款項目金額可留空**（`null` = 待填，跟 0 = 真的 0 元分開） |
| **219** | **請款單送審 → 需求項目變已採購**；刪掉請款項目會退回未採購（補 `on delete set null` 的洞） |
| **220** | **進項發票可從支出帶入**（`tax_invoice.expense_id` ＋ 唯一索引 ＋ `source='expense'`） |

★ 214 不存在 —— 方向寫錯了，在跑之前就刪掉。編號**不補**，
