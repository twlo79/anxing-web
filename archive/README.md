...
## `audits-已結案/` — 查完就結束的診斷（2026-09-05）

29 支，從 `supabase/audits/` 搬過來。

**為什麼搬走**

`audits/` 累積到 50 支，而那個資料夾的用途是「這個月對帳要跑的東西」。
50 支裡有 29 支是**綁定單一事件**的：「Christy 那筆訂單為何前台看不到」、
「那筆 30000 的刪除紀錄在不在」、「pdi_lock 會不會擋住回寫」——
問題查完、修好了，那支查詢就不會再跑第二次。

留在 `audits/` 的 21 支都是**下個月還會再跑一次**的：現金流、收租情況、
押金退還時程、支出明細、契約月租單對帳、找漏帳。

**判斷法**

檔名有沒有指名道姓 —— 人名、日期、單號、金額。有的就是一次性的。

**為什麼不刪**

跟 migration 同一個理由：它們是「當初為什麼會懷疑這件事」的唯一紀錄。
`查-為何刪掉支出沒有稽核紀錄.sql` 的檔頭寫著我當時的猜測**是錯的**，
以及正確答案長什麼樣 —— 那段推理過程 git log 裡只剩一行標題。

★ 這一批裡有兩支特別值得留：`查-採購需求與請款單之間有沒有觸發器.sql`
與 `查-pdi_lock會不會擋住回寫.sql`。它們印出的是**線上實際的觸發器定義**，
而 `schema-baseline.sql` 裡沒有那兩張表（見主 README 的 9.5）。
下次要改採購那條線之前，先重跑那兩支。
