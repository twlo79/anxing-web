...
## `migrations-100-145/` — 第二批封存

45 支，2026-08-22 從 `supabase/migrations/` 搬過來。線上全部跑過了
（`select * from schema_migrations order by name;`），**不要再跑一次**。

**為什麼又搬一次**

跟上次同一個理由:`supabase/migrations/` 又累積到 58 個檔案，
而那個目錄的用途是「這一輪要貼進 SQL Editor 的東西」——
58 個裡有 45 個早就跑過，真正要找的那幾支埋在中間。

這一批涵蓋的主題大致是:房務管理與排班（119–126）、Airbnb 兩階段同步
（112–118）、通知與新訊息（127–130）、`cleaner` 角色（131）、
長租認列改寫（135–138）、採購需求（140–141）、銀行帳戶（142–144）。
這些都已經穩定，改東西時不太需要回頭翻。

**留在原處的是 146 之後** —— 押金移房、押金收款明細、憑證下放、
押金加費那一整條線，2026-08 下旬還在演進，寫新的常常要看上一支怎麼寫的。

**下次什麼時候再搬**

`supabase/migrations/` 超過 40 支就該再切一次。判斷標準不是數量本身，
是「打開這個資料夾找不找得到這一輪要跑的東西」。

---

## `migrations-146-170/` — 第三批封存

25 支，2026-08-27。押金移房與收款明細、憑證下放、事業體帳本、
管家權限、`expenses.request_id`。詳見該資料夾自己的 README。

**留在 `supabase/migrations/` 的是 171 之後**（憑證、訂金與暫收、
付款方式、評價隱藏）—— 那條線還在演進。
...
`migration_100` 之後的當時留在原處：那些是還在演進中的（房務、爬蟲同步、通知）。
**2026-08-22 更新**:那批也穩定了，100–145 已搬到 `migrations-100-145/`。
