...
* 給誰看 —— 只有總經理,還是主管與會計也要
* 期間 —— 本月 / 近 12 個月 / 自訂
* 核心數字:營收、支出、淨額、各物業損益、應收未收、待付款
* **損益要跨越目前的斷層** —— 收入鏈與支出鏈現在各自獨立,要看損益得兩邊各自匯出 Excel 再合併(見 README〈已知缺口〉)
...
做法:抽共用元件(比照 `lib/sortable.tsx`),統一

* 篩選列排版與「清除」的位置
* 日期篩選一律起訖兩欄 + 快捷鈕(本月 / 上月 / 近三月)
* 顯示格式定一套規則寫進共用函式,不要各頁自己 `slice(0, 10)`
