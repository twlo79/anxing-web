...
2. 管家可送請款單，核可要主管以上
3. `migration_139`：只加 select policy，寫入不變。UI 用 `canEdit` 藏掉新增／管理押金／回收桶，下載保留
4. 側邊欄不給 —— **但 RLS 放行**，見下方警告
5. 會計只看得到「收付款帳號」與「常用收款對象」兩個分頁
...
**新開放一個角色看一張表，要動三個地方**（漏一個都不會報錯）：

| 層 | 檔案 | 漏掉的症狀 |
|---|---|---|
| RLS | `supabase/migrations/*.sql` | 畫面一片空白，沒有錯誤訊息 |
| 側邊欄 | `src/app/(app)/layout.tsx` 的 `NAV` | 進得去但找不到入口 |
| UI 動作 | 頁面內的 `canEdit` 之類 | **按了說成功但沒變**（RLS 擋下的 UPDATE 回成功且影響 0 列）—— 最難查 |
