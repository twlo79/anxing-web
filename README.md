...
例：11-12 月期的進項可以選 2026-01-01 ~ 12-31；1-2 月期只能選 1/1 ~ 2/28。

**進項放寬是為了補登** —— 漏掉的進項發票晚幾期才發現，要補在現在這一期申報，
但發票日期是當初開的那一天。鎖在當期的話那張發票就永遠登不進來，
而它代表的是真的可以扣抵的稅額。

**銷項不放寬**：銷項是我方開出去的，開票日就決定申報期，沒有「補在後面那期」這回事。

★★★ **上限一律是該期最後一天，不能填未來的**。未來的發票申報在這一期，
那一期的數字就是錯的，而且會一路錯到留抵結轉的下一期。

★★ 手 key 與「從支出帶入」**共用 `invoiceDateRange()`**，不要各寫一次。
各寫的話會出現「手 key 填得進去、帶入卻看不到那一筆」，而使用者只會覺得系統漏了資料。

★ `<input type="date">` 的 `min`/`max` **擋不住手動輸入** ——
它只把欄位標成 invalid，值照樣寫進 state 然後存進資料庫。
所以 `invoiceDateError()` 在存檔前另外驗一次。

★ 補登**不會重複申報**：`tax_invoice_uniq` 是 `(統編, 進/銷, 發票號碼)`，
**鍵裡沒有期別** —— 同一張發票在別期已經登過的話，這裡會被擋下來。

### 帶入的規則

進項分頁的「⬆ 從支出帶入」列出**日期在上面那個區間內、有填憑證號碼、還沒帶過**的支出。

★★★ 帶進來的**期別是畫面選的那一期**，不是從支出日期推的 ——
...
## 8.2　2026-09-22 ~ 23 這一輪：架構體檢與技術債

> 完整的體檢報告與排程在專案文件 `架構體檢-2026-09-22.md`、`優化排程-2026-09-23.md`。
> 這裡只留**做完的、沒做的、要盯的** —— 三個月後只看這一節就夠。

### 做完的

| 項目 | 做了什麼 | commit / migration |
|---|---|---|
| **多步寫入 → RPC** | 訂金沒收／轉押、備品盤點、需求轉請款、短租移房、請款單存檔，五個「三步請求」改成一支 RPC，任一步失敗整批退回。全部 `security invoker`，RLS 照舊 | `b51f128`，migration 292 ~ 296 |
| **展延留底** | 刪除延展改成還原原本的迄日，不再用月底回推（原本非月底的迄日會被往後挪，最多 30 天，多長一張月租單）| `4e17aed`，migration 297，`lib/contract-extend.ts` |
| **列數檢查** | 核可、駁回、任期收尾、餘額回寫等 11 處決策或錢的寫入接 `.select('id')`，0 列不再當成功。規則收在 `lib/write-guard.ts` | `cb5d59e` |
| **訊息** | 六頁的 `flash()` 改走 `lib/use-flash`：錯誤紅色留到按掉，成功照舊自己走。靠內容判斷（`lib/flash-kind.ts`），呼叫點沒改 | `490c00a` |
| **權限判斷** | 14 處 `role === '…'` 改走 `lib/roles.ts` 的 `isBoss`／`isManager`／`isAccountant`／`isManagerOrBoss`／`isFinance` | `490c00a` |
| **金額格式** | 21 份一模一樣的 `fmt`／`money` 收成 `lib/fmt.ts`（`fmtInt`、`fmtIntOrBlank`）。帶 `$`、不四捨五入的 9 份**刻意留著** | `490c00a` |
| **請假改版** | 三步表單、請整天／請半天／請小時、以小時請自動算起迄（`spreadHours()`）、病假事假改報累積 | `f6d2c9d` ~ `231f30a`，migration 291 |
| **小技術債** | 刪 53 個沒用到的東西、伺服器端日期看台北（`taipeiToday()`）、`contracts` 拿掉 `setTimeout` 等資料庫、死檔搬走、備品與稅務表格可橫向捲 | `07c1a31`，之後一支 |

### 待辦與監控

**活的清單在 `docs/技術債與優化待辦.md`** —— 做完一項就在那裡劃掉，這裡不重複列。
那份分兩段：**待辦**（每一項寫清楚「什麼時候做」，沒有觸發條件的不列）、**監控**（看什麼、門檻、到了做什麼）。
