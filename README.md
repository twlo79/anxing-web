...
|---:|---|:---:|---|---|
| 1 | 出勤 | 🕐 | `/attendance` | 全員 |
| 2 | 房務管理 | 🛎️ | `/housekeeping` | 全員（房務只看得到「行事曆」分頁） |
| 3 | 訂單 \| 收入 | 🛏️ | `/shortterm` | 管家以上 |
| 4 | 契約 \| 收入 | 🤝 | `/contracts` | 管家以上 |
| 5 | 營收表 | 🤑 | `/revenues` | 會計以上 |
| 6 | 請款單控管 | 🧾 | `/purchases` | 管家以上 |
| 7 | 押金管理 | 🏦 | `/deposits` | 管家以上（管家唯讀） |
| 8 | 支出明細 | 💸 | `/expenses` | 會計以上 |
| 9 | 帳戶明細 | 💰 | `/accounts` | 會計以上 |
| 10 | 財務儀錶板 | 📊 | `/dashboard` | 會計以上 |
| 11 | 客戶管理 | 👥 | `/customers` | 管家以上 |
| 12 | 房源評價 | ⭐ | `/reviews` | 管家、主管、總經理（**會計沒有**） |
| 13 | 清潔記錄 | 🧹 | `/cleaning` | 房務、管家、主管、總經理（**會計沒有**） |
| 14 | 設定 | 🔔 | `/settings` | 全員 |
| 15 | 權限管理 | ⚙️ | `/admin` | 會計（兩個分頁）、總經理 |

> **icon 不能撞衫。** 側邊欄收合成只剩圖示時，🏦 押金與 💰 帳戶、🤑 營收與 💸 支出
> 都必須一眼分得出來。帳戶那顆換過三次（🏛️ → 🏧 → 📒 → 💰）才定案。

★ **標案管理**（`/tenders`，2026-08-28）只給 `super_admin`，RLS 也是同一組角色。
...
2. **2026-08-21 起管家可以改訂單與契約的全部內容**，含固定加費、訂單收款、收款證明照片（`migration_154`）。代價要知道：**管家碰得到錢**
...
**收入**：訂單 → 認列（觸發器）→ 營收表
**支出**：請款 → 兩票核可 → 自動產生支出 → 遞延拆成母子單

## 收入

1. 訂單進來（爬蟲或人工）
2. 觸發器 `gen_recognitions` 依 checkin/checkout 拆成月份 → `revenue_recognitions`
3. **長租例外**：`gen_contract_recognitions` 依契約推導，不看訂單怎麼開（`migration_137`/`138`）
4. 收款記在 `order_payments`（短租可分次）或 `orders.paid`（長租一期一個勾）

**平台代收不用記收款**：Airbnb、Agoda、以及 Airbnb 取消收入（`migration_139` 之後）。硬要記只會多出一堆假欠款。

## 支出

1. 建請款單 → 送審
2. **兩票制**：主管一票 ＋ 總經理一票，都到齊才算核可
3. 核可 → 觸發器自動產生 `expenses`
4. **遞延**：一次付一年的保險費會拆成母單 ＋ 12 張子單。母單的 `amount` 是「這一期認列多少」，`gross_amount` 才是實付總額

## 三個一定要記得的

- **名目 ≠ 科目**（`migration_90`/`91`）—— 名目是給人看的，科目是給帳看的
- **辦公室費用不分攤到物業** —— 攤下去的話某一棟會出現它從來沒花過的錢
- **損益是認列基礎，不是收付基礎** —— 「這段期間發生了什麼」，不是「錢進出多少」
...
src/
  ...
  components/RangeInput.tsx      起訖篩選共用元件(全站 9 處)——**連動規則跟輸入框綁在一起**
  lib/date-range.ts              上面那支的邏輯(`.ts` 才測得到)
  components/StatCard.tsx        統計卡共用元件 —— 原本卡片容器有 87 種寫法
  lib/hkParse.ts                 排班解析與計數(**全專案風險最高的邏輯**)
  lib/hk-task.ts                 房務工作的顯示與計數(行事曆)
  lib/hk-payroll.ts              打掃量／布巾／報酬點數 —— 三個數字三種算法
  lib/hk-link.ts                 房務主檔 ↔ ERP 主檔的對應提示(只提示,不自動對)
  lib/hk-import-text.ts          貼上的排班怎麼切成一筆一筆(換行掉光也切得開)
...
voucher_no, no_voucher, shared_voucher, fee_mode, fee_amount,
...
**憑證**(`migration_52` / `155` / `156`):`voucher_no` 與 `no_voucher` 互斥,由 CHECK 約束保證。分成兩件事是為了讓「還沒填」和「本來就沒有」在帳上分得出來 —— 只留一個空白欄位的話,會計永遠不知道還要不要追這張發票。

2026-08-21 起憑證**下放到每個請款項目**。`shared_voucher` = true 時才用單張層級的
`voucher_no`（真的只有一張發票、拆成多個項目報帳）;預設是逐項各填各的。
既有的 59 張全部回填成 `shared_voucher = true`,舊單行為完全沒變。
詳見「五、三條錢的流程」的 5.2。
...
**憑證**(`migration_155`):每個項目各自的憑證號碼與「無憑證」勾勾,互斥
(`pri_voucher_chk`)。母單勾了 `shared_voucher` 時這兩欄**留著但不使用** ——
取消勾選就回來。
...
> **2026-08-22 改成三段式，比照請款單。** 預計匯款日與安幸付款帳號
> 從「送審」移到核可後的「排匯款」—— 送審當下常常還不知道會從哪個戶頭出，
> 以前必填所以大家隨便填一個再回來改，**而改動會清掉核可票、退回重審**。
> 完整說明見「五、三條錢的流程」。
...
### 新訊息:推播的存底,留一週(`migration_128`)

手機鎖屏跳出「新增 3 筆訂單」→ 滑掉 → **那則訊息永遠不見了**。沒有任何地方查得到剛剛那則說了什麼、是哪三筆。

通知的價值在即時,而即時的東西天生會被錯過:開會中、在開車、手機在充電、那台裝置根本沒開推播權限。**錯過一次就等於沒發過。**

所以每一則推播同時寫一份到 `notifications`,在「設定 → 新訊息」看得到。

| 決定 | 為什麼 |
|---|---|
| **一人一列**,不是一則一列 | `read_at` 是每個人各自的狀態。塞進 `recipients` jsonb 的話,兩個人同時標已讀會互相蓋掉 —— 而那個 bug 只在兩人同時開著頁面時出現,重現不了 |
| **先存底,再推播** | `sendToUsers` 原本「沒有訂閱就提早 return」。一個從沒開過推播權限的人會走到那裡就結束,而他才是最需要這一頁的人 |
| **沒有 insert policy** | 前端能寫的話,任何人都能偽造一則通知給自己看 —— 這張表就從「發生過什麼」變成「有人寫了什麼」。只有 service key 寫得進去 |
| **只看得到自己的**(RLS) | 請款審核的通知帶金額與品項。一張全公司共用的消息牌不是這個功能要解決的問題 |
| 點進去**自動標已讀** | 再叫人多按一次「標為已讀」,那顆按鈕不會有人按,然後所有東西永遠是未讀 |
| 側邊欄「設定」旁顯示未讀數 | 打開 app 看不出有東西等著,等於存了也沒用。收起來的側邊欄只剩 emoji,那時改成一個紅點 —— 位置有限時「有沒有」比「幾則」重要 |

**「留一週」是兩件事,分開做:**

| | 做什麼 | 不這樣做會怎樣 |
|---|---|---|
| 讀取 | 只撈七天內 | 只靠週日清的話,週六會看到十三天份 ——「留一週」變成看心情的數字 |
| 清除 | `pg_cron` 每週日凌晨 4 點(台北)刪七天前的 | 只靠讀取過濾的話資料永遠不會刪 —— 十個人、一天十則,一年三萬多列躺在那裡 |

排程在資料庫裡(`migration_129`),不是外部呼叫 —— 清資料本來就屬於資料庫,送出去繞一圈只多了三個會壞的環節(網路、金鑰、外部排程器)。排程前先 `unschedule` 同名的:不然重跑 migration 會排出第二個,兩個都跑而 `delete` 是幂等的,**完全看不出異狀**。

備用的手動入口是 `POST /api/notifications/purge`(要 `x-import-key`)。
...
「房務行事曆」原本掛在**出勤**底下,而它跟出勤沒有關係 —— 出勤講「員工幾點上下班」,行事曆講「哪一間房什麼時候要清、誰去清」。放錯地方的代價不是不好看,是**找不到**。

現在兩個都在「房務管理」底下,同一件事的兩個視角:

| 分頁 | 回答什麼 | 給誰看 |
|---|---|---|
| 行事曆 | 這個月每天誰在哪 | 全員(要配合排班的人) |
| 排班統計 | 各房源幾間幾次、布巾多少、報酬多少 | 主管以上 |

行事曆排第一,而且**全員可見** —— 搬過來時如果整頁鎖給主管,管家就看不到自己的班表了。那不是「權限收緊」,是功能消失,而且沒有任何提示。

### 工作從訂單長出來(`migration_121`)

`hk_task` 一筆 = 一件要做的事(哪天、哪間、什麼工作、誰做)。退房清潔與入住清潔由 `hk_sync_order_tasks()` 觸發器從 `orders` 推導 —— **訂單改日期,工作跟著搬,但保留指派**。

| 決定 | 為什麼 |
|---|---|
| 格子裡寫「退房清潔 A15・Kevin」,不是「庭玉 1・Una 2」 | 「庭玉 1」回答的是「誰在」,但看行事曆的人要問的是**今天要做什麼**。看不出是哪間、退房還是入住、客人是誰,就得點進去 —— 而一個月三十天每天都要點進去的行事曆等於沒有 |
| 自動長出來的畫**灰色**、明寫「自動填入」 | 用人的顏色會讓人以為「有人安排過了」,然後那些沒指派的就一直沒人去做。灰色 = 這是系統猜的,還要你指派。指派之後換成那個人的顏色 —— 顏色的變化本身就是進度 |
| 自動產生的**不能直接刪** | 它跟訂單連動。刪掉那條線就斷了,之後延住兩晚不會有人知道 |

### TimeTree 排班怎麼進來

TimeTree 是人在日曆上打字排的班,`hk_work_item` 存「誰做」,`hk_task` 存「哪天哪間」—— 兩份資料各有一半,正好互補。

```
貼上文字 / JSON  →  hk_event / hk_work_item / hk_day  →  hk_apply_timetree()  →  hk_task 的指派
   （匯入面板）        （全刪重建，一次一個月）              （只覆蓋指派，不刪工作）
```

**匯入按鈕在「行事曆」,不在排班統計**(2026-08-14 使用者指定)。匯入改變的是行事曆上的格子 —— 按完當場就看到灰色變成各人的顏色,那個變化本身就是「成功了」的證據。放在統計頁的話,按了一顆按鈕、另一頁變了,而人不會回去看另一頁確認。

匯完**自動跑 `hk_apply_timetree`**。分開的話他會匯完發現行事曆沒變,以為匯入失敗,然後再匯一次。

| 規則 | 為什麼 |
|---|---|
| **只覆蓋指派,不刪任何工作** | TimeTree 的月曆每天只顯示前幾筆,其餘收成「+3」,抓取要逐一點開。漏掉的那幾筆如果先把自動產生的刪光就**永遠不見了**,而且沒有任何跡象 |
| 一次只能匯一個月 | `period` 是全刪重建的單位。兩個月混在一批送出去,其中一個月會被另一個月整批覆蓋,而畫面上的數字看起來完全正常 |
| 看到日期就切一筆(不靠換行) | 實際貼進來的換行是掉光的。`split('\n')` 的結果是**一筆**,而那一筆的負責人欄是後面整個月 —— 預覽顯示「共 1 筆、未知人員 SHAO-YING HSIEH」,看起來像人員主檔的問題,其實解析從第一步就散了。切法與測試在 `lib/hk-import-text.ts` |

### 房務主檔要對到 ERP 主檔(`migration_124`)

第一次試算:100 筆排班只套上 10 筆。不是資料壞了,是**兩邊的主檔從來沒有對過** —— 房務那邊叫「開4」,ERP 那邊叫「開封4F」,而 `migration_122` 是拿名字去猜的。

```
hk_property.property_id  →  properties     （開4 → 開封4F）
hk_staff.staff_id        →  staff         （庭玉 → 謝庭玉）
```

**對應關係是事實,不是規則。** 再補幾條「去掉 F」「把開補成開封」的猜測規則,錯的時候不會有人發現:「台2」猜成「台視2」聽起來合理,但如果 ERP 那邊其實叫「台北2」,工作就被指派到另一間房 —— 排班表看起來滿滿的,實際上有人去清一間不用清的房,而該清的沒人。

所以回填**只填完全同名的**,其餘留空。少填一個是「有幾筆要人來對」,看得到;填錯一個是「工作被指派到別間房」,看不到。兩種錯不對等。

介面在「房務管理 → 設定」,選單把對得上的排到最前面,底下給一行「是不是「開封4F」?」—— 但**只有唯一一個對得上才提示**。「開2」同時像 開封2-1、開封2-2、開封2F,那三個不提示:一個看起來很有把握的錯誤提示,比什麼都不給更容易被按下去。規則與測試在 `lib/hk-link.ts`。

### 打掃報酬:三個數字三種算法

`lib/hk-payroll.ts`(20 個測試)。**這三個不能共用同一個數字**,而共用的症狀是薪水或叫貨量差一倍:

| 數字 | 算法 | 為什麼不同 |
|---|---|---|
| 打掃量 | 一份工除以人數(兩人合掃各 0.5) | 兩個人分一份工 |
| 布巾組數 | 一份工算**一間**,不除 | 房間只有一張床,不會因為兩個人去就變兩張 |
| 報酬點數 | 打掃量 × 房源的 `clean_points` | 開封4F 爬四層樓就是比 A15 累,誰去掃都一樣 |

幾個容易寫錯的邊界,測試各釘一條:

* **退房與入住是兩份工**,同一天同一間會有兩筆 —— 算成合掃的話兩筆各變 0.5,總量憑空少一半
* **同一個人重複指派只算一個人** —— 不然他自己跟自己「合掃」
* **不在這一層四捨五入** —— 每筆都進位的話,一個月幾十筆會累積出好幾間的差,而那個差直接變成多發的薪水
* **房源沒設點數要另外報,不能當成 0** —— 當成 0 的話那個人那筆白做了,而他要自己對帳才會發現

### 打掃點數掛在房源,不掛在人(`migration_123` / `124`)

難度是房子的性質。掛在人身上的話,每換一個人負責就要重設一次 —— 而漏設的那次不會報錯,只會讓那個月的報酬少一截。

**整棟／整層不手填,由子房源加總**:開封整棟 = 2F + 3F + 4F = (3+2) + 3 + 4 = 12。手填一個 12 的話,之後有人把 4F 從 4 調成 5,整棟還是 12 —— 那 1 點的差每個月少發一次,沒有人會發現。子代有任何一個沒填就維持 `null`(算不出來,不是 0)。

> 加總由觸發器維護,而觸發器會更新 `clean_points` —— 也就是**呼叫自己**。重入用交易內旗標擋(`set_config('app.recalc_points', …, true)`);`pg_trigger_depth()` 擋不乾淨,因為從 SQL 直接呼叫時深度是 0,第一層觸發器仍會再進來一次。第一版沒擋,直接 `stack depth limit exceeded`。

### 房源的包含關係(`migration_119`)

`properties.parent_property_id`:開封2-1 → 開封2F → 開封整棟。

原本用「整棟 vs 同物業其他房源」判斷空間重疊,被開封證偽 —— 開封店面、1F-1 不在整棟裡卻會被誤標,而開封2F vs 2-1 是真的撞房卻抓不到。改成祖先鏈之後,只比有包含關係的房源對,兄弟不算,退房那天不算重疊。

防環用**觸發器**不是 CHECK:CHECK 只看得到一列,看不到 A→B→C→A。
...
### 範圍對了還不夠:宣告不等於做到（migration_125）

修好 scope 的定義之後,2026-08-15 又冒出 46 筆誤報,裡面有當天正要入住的 Ryan Collin 與 Conrad Chan —— 它們就在同一天的房務排班表上。範圍是對的,是**那一趟沒跑完**:登入過期、翻頁斷在一半、網路抖一下,結果都一樣。

兩道防線,都不需要爬蟲多做什麼(規則與測試在 `lib/airbnb-sync.ts`):

| 防線 | 門檻 | 為什麼 |
|---|---|---|
| 一輪掉太多就整批不算 | 掉超過範圍內 10% 且 ≥ 5 筆 | 訂單真的從 Airbnb 消失是稀有事件。一天 46 筆的合理解釋永遠是「這次沒抓完」,不是「46 組客人同時退掉」 |
| 連續兩輪沒看到才標記 | `miss_streak >= 2`,看到就歸零 | 偶發的抓取不全撐不過第二輪 |

兩道都往「寧可晚一天報」倒:漏報一筆是晚一天發現;誤報 46 筆是這份清單再也沒有人看,連真的那一筆也被埋掉。

### 一間房會有好幾個 listing_id(`migration_127`)

`migration_126` 的自檢列出 47 個卡在停用房源上的編號,而裡面有:
...
同一間房在 Airbnb 上被重建過好幾次,每次換一個新編號。**這是事實,不是資料錯誤。**

而 `properties.airbnb_listing_id` 是一個欄位加唯一索引 —— 一間房只放得下一個。所以「把 listing_id 搬到現行房源」只是把問題往前挪一格:A18 拿了其中一個,另一個編號的訂單照樣對不到房源,而**對不到的訂單整筆不會進系統**,報表看起來完全正常。

所以改成多對一:

```
property_listings
  listing_id   Airbnb 房源編號（主鍵 —— 一個編號只能指一間房）
  property_id  指向現行房源
  is_current   現在還掛在 Airbnb 上的那一個
```

對帳、訂單匯入、評價匯入三處都改讀 `listing_property_map`。各寫一次比對邏輯的話,之後多一條規則要記得改三個地方 —— 而漏掉的那個不會報錯,只會安靜地少掛一批訂單。

**回填只填「去掉舊字之後完全同名」的**(`舊-A18 → A18`、`舊A11(1083) → A11`)。`舊-未知(0164)`、`整棟(新)` 這種對不上的留空,在「權限管理 → 房源管理」按「＋ 加一個舊編號」手動補。猜錯的後果是那個 listing 的訂單掛到別間房 —— 營收算到錯的房東頭上,而兩邊的數字看起來都合理。

> **為什麼不把舊房源改回啟用就好?** 那樣訂單確實會進來,但進的是**舊-A18 那一列房源**:同一間房的錢分在兩個地方,房東對帳對不起來,房務工作也長在一間沒有人會看的房源上。而「停用 = 這間房不營運了」這個意思也跟著失效。

### 為什麼訂單對到停用房源就不進系統

停用等於「這間房不再營運」。訂單掛上去的話,那筆錢會出現在一個沒有人在看的房源底下 —— 不在現行房源的營收表裡、不會長出房務工作、房東對帳也對不到。

**那比不進系統更糟**:不進系統至少會產生一條「對不到房源」的建議,掛到舊房源則是一片安靜。
...
**唯一不在 Cowork 上的排程**是 `purge_old_notifications`(每週日凌晨 4 點台北時間,清七天前的通知存底)—— 它跑在資料庫裡(`pg_cron`,`migration_129`)。因為那件事完全不需要瀏覽器:清資料本來就屬於資料庫,送到外部繞一圈只多了三個會壞的環節(網路、金鑰、外部排程器),而且電腦關機也照跑。
...
| ~~登入頁密碼管理員認不出欄位~~ | **已修**:補上 `name` / `id` / `autoComplete="username"` 與 `"current-password"`。原本瀏覽器只能用猜的,有時不問要不要儲存、有時把 email 填到密碼欄。另外加了眼睛切換、大寫鎖定提示,並把上次登入的 **email**(絕不含密碼)記在 localStorage |
...
| **三支 lib 有測試但沒有畫面在用** | `annual-leave`（特休額度）、`hk-calendar`（房務行事曆檢視）、`pnl`（三段式損益）。邏輯與測試都是真的,只是**沒有任何 `.tsx` import 它們**（2026-08-27 盤點）。<br>★★ 危險的地方是**測試全綠會讓人以為功能好好的** —— 跟 9.2 那條「欄位有 FK、有索引、前端在讀,但從來沒有人寫過它」同一種錯覺。三個檔頭都加了警語,接上畫面時記得拿掉 |
...
**跑過的已經封存,不在 `supabase/migrations/` 裡**（最近一次 2026-08-27）:

| 編號 | 現在在哪 |
|---|---|
| 28–29 | `archive/migrations-pre-30/` |
| 30–99 | `archive/migrations-30-99/` |
| 100–145 | `archive/migrations-100-145/` |
| 146–170 | `archive/migrations-146-170/` |
| **171 之後** | **`supabase/migrations/`（還在演進中）** |

★ `src/` 與 `docs/` 的註解裡還有很多「（`migration_147`）」這種引用 ——
**那些沒有失效**,它們是「這個行為是哪一支訂下來的」的指路標,現在指到 archive。
搬走的是檔案,不是那個編號的意義。

那個目錄的用途是「這一輪要貼進 SQL Editor 的東西」——
跑過的留著只會讓真正要找的那幾支埋在中間。
線上跑到哪一支查 `select * from schema_migrations order by name;`。
...
| 117 | 清掉 203 筆「在 Airbnb 找不到」的誤報 —— `scope` 送的是三趟抓取的 min/max,而 min/max 是「碰過哪些日期」不是「掃遍了哪些日期」 |
| 118 | 建議可**忽略／一鍵套用**(`dismissed_at/by/sig`、`sync_issue_log`、`dismiss_sync_issue`、`apply_sync_issue`)。忽略記 sig —— 同一條建議的內容變了要重新出現 |
| 119 | `properties.parent_property_id` ＋ **防環觸發器** ＋ `property_ancestors()`。CHECK 看不到其他列,擋不了環。原本用「整棟 vs 同物業其他」判斷空間重疊,被開封證偽:開封店面不在整棟裡卻被誤標,開封2F vs 2-1 是真撞房卻抓不到 |
| 120 | 套用住宿起訖時**一併寫入金額** —— 延住不可能只改日期不改錢 |
| 121 | **房務工作** `hk_task` ＋ `hk_sync_order_tasks()` 觸發器:退房/入住清潔從訂單長出來,訂單改日期工作跟著搬但**保留指派** |
| 122 | `hk_apply_timetree()`:把 TimeTree 的「誰做」套到 `hk_task`。**只覆蓋指派,不刪任何工作** —— TimeTree 的抓取可能不完整,先刪再補會把漏掉的永遠弄丟 |
| 123 | `properties.clean_points`(打掃難度係數)。報酬點數 ＝ 打掃量 × 點數。名稱對不上的**留 null 不猜** —— 猜錯的方向是少發薪水 |
| 124 | **房務主檔對到 ERP 主檔**:`hk_property.property_id`、`hk_staff.staff_id`。100 筆排班只套上 10 筆,因為 122 是拿名字猜的(房務叫「開4」、ERP 叫「開封4F」)。回填只填完全同名的 —— 少填看得到,填錯是「工作被指派到別間房」而沒有人會發現。順帶:整棟/整層的 `clean_points` 由子代加總,**重入用交易內旗標擋**(觸發器會呼叫自己,`pg_trigger_depth()` 擋不乾淨) |
| 125 | `airbnb_snapshots.miss_streak`:**連續兩輪沒看到才算消失**,一輪掉超過 10% 判定為抓取不完整整批不算。46 筆誤報裡有當天正要入住的客人 |
| 126 | `move_listing_id()`:一個交易做完「清舊的」與「設新的」。分兩次做的話中間失敗會變成兩邊都空,而那時連提醒的建議都沒了 |
| 127 | **`property_listings`**:一間房可以有多個 Airbnb 編號(A13 有四個)。房源重新上架就換一個新編號,而 `properties.airbnb_listing_id` 只放得下一個 —— 放不下的那個編號,它的訂單**整筆不會進系統**。對帳、訂單匯入、評價匯入都改讀 `listing_property_map` |
| 128 | **`notifications`**:推播存底留一週。一人一列(不是一則一列)因為 `read_at` 是各自的狀態。**沒有 insert policy** —— 前端能寫的話任何人都能偽造一則通知給自己看 |
| 129 | `pg_cron` 每週日凌晨 4 點(台北)清七天前的通知。排程前先 `unschedule` 同名的 —— 不然重跑會排出第二個,兩個都跑而 `delete` 是幂等的,**完全看不出異狀** |
| 130 | 評價統計納入停用物業 —— 房源停用不代表它的歷史評價該消失 |
| 131 | **新增「房務」角色**,「一般」改名「管家」。⚠ `cleaner` 在 RLS 眼裡跟 `housekeeper` 相同,只是選單收窄 |
| 132 | 出勤表拆成「應到」與「實到」 |
| 133 | 自動長出來的工作改成**建議**,打勾才進行事曆 —— 系統負責看見,人負責決定 |
| 134 | 房務工作加標題與時間(照 TimeTree) |
| 135 / 136 | 月租單改成跟著**契約週期**走(7/16 起租的第一期是 `7/16~8/16`)/ 既有月租單的期間一起搬 |
| 137 / 138 | 長租認列改由**契約**推導,不再跟著訂單走 / 認列真正脫離訂單(`order_id` 可為 null、加 `contract_id`) |
| 139 | 管家與房務**看得到押金但改不了**(只加 select policy) |
| 140 / 141 | 採購需求單 / 加寄送地點、拿掉數量 |
| 142 / 143 / 144 | 帳戶管理:三個銀行帳戶的流水鏡像 / 餘額我們自己算,有差異才備註 / 清掉舊解析器的流水準備重匯 |
| 145 | 一次性費用也要有「項目」 |
| 146 | **押金移房**:A 房的押金整筆轉到 B 房,兩邊互相標記。只允許等額、只有會計與總經理能轉 |
| 147 | **押金收款一筆一列**(比照訂單)。押金常分兩次收,舊模型記不下第一筆 —— 「收了一半」跟「一毛沒收」以前長得一樣 |
| 148 | 會計科目加「運費」,順手補上「水電瓦斯」對不到科目的**靜默錯誤** |
| 149 | 退回草稿時**由資料庫清票**,前端不再碰核可欄位 |
| 150 / 151 / 152 | 「主管按確認付款日沒反應」查了一整天。真因是 `ON CONFLICT (fee_request_id)` **對不到部分索引** —— 150 是誤判(那顆索引早就有),151 才是真的修法,152 清掉多建的那顆。⚠ 教訓:先把錯誤訊息弄出來,不要一直推理 |
| 153 | **全員可讀 `profiles` 姓名**。以前只有總經理看得到,其他人的「請款者」整欄是「—」,而那跟「真的沒填」長得一樣 |
| 154 | **契約與訂單編輯全面開放給管家**:固定加費、訂單收款、收款證明照片。代價是管家碰得到錢 |
| 155 / 156 | **憑證下放到請款項目** ＋「共同憑證」開關。以前十七張收據用頓號串成一串塞進同一格,再整串複製給每一筆支出 —— 計程車那筆的號碼裡混著住宿的發票號 |
| 157 | **押金加費**:應退 = 押金 − 加費。加就是 `orders` 的 oneoff 子單多一個 `deposit_id`,營收報表不用改。押金退掉後整張訂單鎖住 |
| 158 | 每筆加費各自可附照片(`attachments.order_id`,前綴 `of/`)。掛在押金底下的話分不出哪張對哪筆 |

**108 不存在** —— 編號跳過,不是遺失。

**30–99 已封存到 `archive/migrations-30-99/`**(2026-08-15)。線上全部跑過了,搬走只是為了讓 `supabase/migrations/` 回到「這一輪要貼的東西」——100 個檔案裡有 97 個已經跑過,真正要找的那幾支埋在中間。

**沒有合併成一支 baseline**,而且刻意不合併:`schema_migrations` 記的是檔名,合併之後「線上跑到哪了」就再也回答不了;而且 migration 的價值不只是「建了什麼」,是**「為什麼這樣建」**—— 112 講的是重複訂單、117 講的是 203 筆誤報怎麼來的,合併成 DDL 之後那些全部消失。要「現在的樣子」請看 `supabase/schema-baseline.sql`。
