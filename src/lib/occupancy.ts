...
export type RoomOcc = {
  ...
  /** 空著的天數 ＝ days − used */
  free: number;
  /** 入住率 0 ~ 1。`days` 是 0 的時候回 0，不是 NaN */
  rate: number;
};
 ...
 *   會把整體入住率一路往下拉，而畫面上完全看不出原因。
 *
 * ★ 排序照房號的自然順序（10 排在 9 後面），不是按入住率。
