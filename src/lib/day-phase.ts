...
export type Phase = {
  ...
  ink: CardInk;
  /**
   * 背景的類別名稱，定義在 `globals.css` 的 components layer。
   *
   * ★★ 2026-08-28 從 Tailwind 的 `bg-gradient-to-br from-[…]` 換過來。
   *
   *   新的霧面需要**多層背景**（底層線性漸層 ＋ 三顆色霧 ＋ 兩顆白光暈），
   *   而 Tailwind 的漸層工具只給得起一層。
   *
   * ★ 換過來順便消掉了原本那個風險:Tailwind 靜態掃描原始碼,
   *   組出來的類別不會被產生 —— 畫面上變成沒有背景,而且編譯不報錯。
   *   類別名稱寫死在 CSS 裡就不可能發生。
   */
  gradient: string;
};
 ...
 * 【一天的光線】（使用者選的）
 *
 *   早上 霧藍 → 中午 橘霧 → 午後 過渡 → 晚上 深霧藍
 *
 * ★★ 2026-08-28 改成霧面（參考「沉積層」展覽主視覺）:低飽和、高亮度,
 *    顏色像霧一樣浮著,而不是一塊實色。
 *
 * ★ **沒有橫線。** 前一版試過極細的水平沉積紋,拿掉了 ——
 *   那些線跟卡片上數字的基線平行,眼睛會把它們當成表格的分隔線,
 *   而卡片上並沒有一行一行的東西。留下純粹的霧與光暈。
 *
 ...
 * 【白天配深字、晚上配白字】
 *
 * 早上、中午與午後都是淺色的 —— 白字放上去只有 1.8:1 / 1.7:1 / 1.4:1，
 * 等於整張卡的時間消失。而打卡是站在戶外做的事，太陽底下更慘。
 *
 * ★ 2026-08-28 降飽和之後,傍晚也從深色變成淺色 —— 四段裡有三段配墨色字。
 *
...
const PHASES: Record<PhaseKey, Phase> = {
  morning: {
    ...
    // 清晨的天空。霧藍:近乎白的藍裡浮著一層更深的藍
    gradient: 'phase-morning',
  },
  afternoon: {
    key: 'afternoon', greeting: '午安', icon: '🌤️', ink: ON_LIGHT,
    // 正午。橘霧:比原本的黃橘淡很多,飽和度降下來才跟另外三段是同一套語言
    gradient: 'phase-afternoon',
  },
  evening: {
    /*
     * ★★ 2026-08-28 從 ON_DARK 改成 ON_LIGHT。
     *
     *   舊的暮藍（#6D89D2 → #4A61A6）夠深,白字讀得到。
     *   新的過渡色上緣是 #EFD8BE —— 幾乎是米色,白字放上去大約 1.4:1,
     *   **那個時間直接消失**。降飽和的代價要在這裡付。
     */
    key: 'evening', greeting: '傍晚好', icon: '🌆', ink: ON_LIGHT,
    /*
     * 午後過渡：橘往藍走的路上，留著一點紫。
     *
     * ★ 橘直接跳到深藍會像換了一張卡 —— 這一段是那條線的中間。
     *
     * ★ 再往藍走的話會跟晚上的夜空藍變成同一個色相，兩段擺在一起
     *   像同一個顏色的深淺 —— 而傍晚與晚上要分得開：
     *   一個是天還沒黑，一個是天黑了。那點紫就是差別。
     */
    gradient: 'phase-evening',
  },
  night: {
    key: 'night', greeting: '晚安', icon: '🌙', ink: ON_DARK,
    // 夜空。深霧藍 —— 跟主色卡 .surf-deep 同一個家族,只是更暗
    gradient: 'phase-night',
  },
};
