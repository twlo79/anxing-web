...
test('跨月時第二行是訂單起訖', () => {
  const x = periodCell(r({
    checkin: '2026-07-26', checkout: '2026-08-01',
    period_start: '2026-07-26', period_end: '2026-07-31',
  }));
  assert.equal(x.main, '2026-07-26~07-31');
  assert.equal(x.sub, '2026-07-26~08-01');
});
...
test('舊資料沒有認列起訖時退回訂單起訖，不是空白', () => {
  ...
  assert.equal(x.main, '2026-07-25~07-29');
