 ...
 *
 * ══════════════════════════════════════════════════════════
 * 【使用者 2026-09-18】「申報 和 備註 資訊放一起」
 *
 * ★★★ 這一行講的是**這份報表本身**（哪一期、什麼時候送出去的、備註）。
 *   第三行講的是**這個檔案**（誰傳的、叫什麼、多大、什麼時候進來的）。
 *   原本申報日放在第三行 —— 跟上傳日排在一起看起來很整齊,
 *   但那是「兩個日期」的整齊,不是「同一類事情」的整齊。
 *
 * ★★ 空的那一段**整段不見**,不是印一個講不出自己是什麼的「—」。
 * ★★★ **備註不在這一行**（2026-09-18 使用者選的甲）——
 *   它自己一行、橫跨整列。擠在這裡只有約 180px，稍長就被截掉,
 *   而備註是自由輸入的,多長都有可能。
 * ══════════════════════════════════════════════════════════
 */
 ...
 * 備註那一行的標籤。
 *
 * ★★★ 備註**自己一行、橫跨整列**（2026-09-18 使用者選的甲）。
 *   原本擠在期別後面只有約 180px，稍長就被截成「⋯」——
 *   而備註是自由輸入的，多長都有可能。獨立一行有 870px。
 * ★ 前面要有「備註」兩個字 —— 那一行單獨出現時，
 *   看的人不知道那是什麼（一段沒有名字的灰字可能是任何東西）。
 */
export const NOTE_LABEL = '備註';

/**
 * `2026-09-18` → `2026/09/18`。
 *
 * ★★★ **年份要印**（2026-09-18 使用者:「顯示年月日」）。
 *   原本只印 `09/18` —— 這幾頁的東西會放好幾年，
 *   明年再看就分不出是今年傳的還是去年傳的。
 * ★ 西元，不是民國（2026-09-18 使用者選的甲）。
 *   同一行上的「115年 7-8月」是**期別**的民國年;
 *   申報日再寫成 115 的話，**兩個 115 意思不一樣**，看快了會混。
 */
const ymd = (d: string): string => String(d).replace(/-/g, '/');
...
export function fileLine(p: {
  ...
  const parts = [
    (p.who ?? '').trim(),
    /*
     * ★★★ **檔名不印**（2026-09-18 使用者:「檔名不要」）。
     *   下載下來的檔名早就是**標題**那一行（`reportFileName`）——
     *   存在資料庫裡那個原始檔名沒有人會用到，
     *   而它是這一行裡最長的東西（`正隆_收支報表_115年8月_FINAL_v4.xlsx`）。
     */
    (p.size ?? '').trim(),
    /*
     * ★★ 申報日**不在這一行**（2026-09-18 使用者:「申報 和 備註 資訊放一起」）。
     *   這一行講的是「這個檔案」:誰傳的、叫什麼、多大、什麼時候進來的。
     *   申報日講的是「這份報表的事」—— 它跟期別、備註同一類，在上面那一行。
     */
    p.uploadedOn ? `上傳 ${ymd(p.uploadedOn)}` : '',
  ].filter(Boolean);
