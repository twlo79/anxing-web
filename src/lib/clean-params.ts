 ...
 *
 * ============================================================
 * 【★★★ 只收 0.5 或 1】（2026-09-03 使用者:「只有 0.5 or 1 可以合併記，其它不行」）
 *
 * 這個欄位不是「精確的房間數」，是**概算估計** ——
 * 一個人一份工大概做了半天份還是一天份。
 *
 * 以前什麼數字都收（0.17、4⋯），而那讓人以為它是精確的:
 *   正隆一份工掃三間、工作量算一半 → 我一度拆成 0.17／0.17／0.16 三筆。
 *   使用者:「這些都正確**是概算估計喔**，不要以間數算」——
 *   細分到小數點後兩位只是把一個估計值裝成精確值。
 *
 * ★ 留空 = null = 這是行程來的正常一份工（合掃由 `eachShare` 自己各半）。
 * ★★ 有值（0.5 或 1）= **人手動記的**，見 `isEstimatedJob`。
 */
export const UNIT_CHOICES = [0.5, 1] as const;

export function parseUnits(raw: string): ParseResult {
  const r = parseNum(raw, 2, '間數');
  if (!r.ok || r.value == null) return r;
  if (!(UNIT_CHOICES as readonly number[]).includes(r.value)) {
    return err('間數只能填 0.5 或 1 —— 那是概算，不是精確的房間數。'
             + '金額請填在旁邊的「金額」欄');
  }
  return r;
}
 ...
 * 【★★★ 為什麼這件事決定床單算不算】
 *
 * 使用者的原話:「主要是無法記錄出要幾個床單，**因為清三間不一定有換床單**」。
 *
 * 正常一份工（行程來的、`units_override` 是 null）:
 *   一間房打掃一次 → 床單 = 那間房的床數。推導得出來。
 *
 * 合併記的一份工（人填了間數）:
 *   「今天在正隆掃了三間、大概半天份」—— 換了幾組床單**沒有人知道**。
 *   照床數去乘只是編一個數字出來，而那個數字會被當成真的。
 *
 * ★ 所以:有值 → 床單不自動算，要在「拿床單」那欄手動填（不填就是 0）。
 * ★★ 判斷的是「有沒有值」不是「值是多少」—— 0.5 跟 1 一樣都是人估的。
 */
export const isEstimatedJob = (unitsOverride: number | null | undefined) =>
  unitsOverride != null;
