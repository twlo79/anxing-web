...
export const FEE_TYPES = [
  ...
  '運費',
  /*
   * 【稅費】（2026-09-02 使用者:「若要開發票，單次每期加稅費，開立發票」）
   *
   * 契約勾了「價格未稅」的，租金本身不開發票。要開的話就在那一期
   * 加一筆稅費 —— **那一筆才是發票的依據**。
   *
   * ★ 叫「稅費」不叫「營業稅」（使用者指定，已進 CLAUDE.md 的用語表）。
   * ★★ 它是**收入**不是支出:我們向房客收的，跟支出頁的稅金無關。
   */
  '稅費',
  '其他',
] as const;
 ...
 * 那個選項存在本身就是在邀請人填錯。
 */
export const ONEOFF_ONLY_FEE_TYPES = [
  '保證金',
  /*
   * 房務（migration_229）。安幸對物業收的服務費與人事費。
   *
   * ★★ 放這裡不放 `FEE_TYPES` —— 它們只會由房務產生器開,
   *   放進共用清單的話會出現在「每月自動產生」的選單上,
   *   而一個每月自動長出來的房務清潔在會計上講不通。
   *
   * ★★★ **資料庫的 `order_account_code()` 一定要一起改**（migration_229）。
   *   只加這裡的話,那筆收入在營收報表上會掉進「其他」——
   *   而金額是對的、名目也是對的,只有分組錯了,沒有人會發現。
   *   （migration_148 加運費時就是這樣踩過一次。）
   */
  '房務清潔',
  '人事費',
] as const;
...
export const CONTRACT_FEE_PRESETS: { label: string; fee_type: string; item_name: string | null }[] = [
  ...
  // 未稅契約要開發票時加的那一筆（2026-09-02）
  { label: '稅費',          fee_type: '稅費',   item_name: null },
