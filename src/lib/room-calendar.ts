 ...
 *
 * ★★★ 「幾天內」**不在這裡**。2026-09-16 一整天走了三個版本:
 *     45 ／ 7　　寫死　→ 70 天與 106 天到期的契約看不到
 *     180 ／ 14 寫死　→ 每天打開都是一整頁半年後的事
 *     可以按的 → 使用者:「不要有 hardcode 什麼時候」
 *
 *   所以門檻搬到 `lib/hk-alerts.ts` 的 `ALERT_WINDOWS` ／ `DEFAULT_WINDOW`，
 *   **房源狀態與未來提醒兩頁共用同一排膠囊**。
 *   這裡只留「怎麼算」，不留「算到哪裡為止」。
 *
 * ★★ 原本的 `ENDING_DAYS` ／ `LEAVING_DAYS` 已經刪掉 ——
