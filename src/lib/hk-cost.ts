...
export function cleaningCosts(
  ...
  /*
   * ══════════════════════════════════════════════════════
   * 【★★★ 曾經有一條「時薪人員獨做的工不產生清潔費」的排除，
   *    2026-09-09 拿掉了。這段留著是因為它會很想被加回來。】
   *
   * 那條規則在**舊模型**裡是對的:劉姐的工資記在**被打掃的那間房**上,
   * 所以再向物業收一次清潔費 = 同一份工向物業收兩次。
   *
   * 2026-09-07 改成成對分錄之後模型變了 ——
   * 她的工資記在**安幸辦公室**,不是物業。於是一份工變成三筆:
   *
   *     ① 物業      支出  房務清潔（間數計）
   *     ② 安幸辦公室 收入  房務清潔（間數計，與 ① 同額反向）
   *     ③ 安幸辦公室 支出  薪資勞務（時薪計）
   *
   * ①② 是物業付給安幸的服務費，③ 是安幸付給劉姐的工資 ——
   * **兩件事互不相干,沒有重複。** 排除規則留著的後果是
   * 「安幸付了工資卻沒有向任何人收錢」,而畫面上只是少了幾列,
   * 總額看起來很正常（2026-09-09 使用者抓到:
   * 「劉姐 有產生支出 但沒有房源請款」「8/1 18B5、8/28 14B5 9000」
   *  「復興 也有少 清一次 請 730」）。
   *
   * ★ 要再加回類似的排除之前，先確認**工資記在誰身上**。
   * ══════════════════════════════════════════════════════
   */
  for (const j of jobs ?? []) {
    /*
