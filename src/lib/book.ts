...
/**
 * 換帳本時，「安幸代墊」那顆勾要變成什麼（2026-09-17 使用者指定：
 * 「之後 愛皮 洪鯊 都自動連動 把安幸代墊 自動打勾起來」）。
 *
 * ══════════════════════════════════════════════════════════
 * 【為什麼要自動勾 —— 這是漏了一年的那一勾】
 *
 * 2026-09-17 查出來:有請款單的支出 195 筆，而代墊暫付只有 **2 筆**。
 * 愛皮／洪鯊的單走的是安幸的戶頭，但那顆勾是**手動**的 ——
 * 沒勾的話畫面上一切正常（支出記進愛皮、金額對、單也送得出去），
 * **只有安幸那邊少記一筆應收**，而少記的那筆不會有任何地方報錯。
 *
 * ★★★ 這正是「預設值站錯邊」:要做對事得多按一下，
 *   而忘記按的代價是一筆看不見的應收。反過來才對 ——
 *   自動勾起來，真的不是代墊的那幾張再讓人手動取消（勾還在，點得掉）。
 *
 * ══════════════════════════════════════════════════════════
 * 【★★ 只在「換帳本」時算，不在開啟舊單時算】
 *
 * 呼叫點是使用者**自己動了事業體那顆下拉**的時候。
 * 如果改成每次開單都重算，打開一張舊單、什麼都沒改按存檔，
 * 就會安靜地替它補上一筆應收 —— 那是資料被改了而沒有人按過任何東西。
 * 既有的單要不要補，是另一件事（要先確認那些錢是不是走零用金出的）。
 *
 * ══════════════════════════════════════════════════════════
 * 【★ 暫支款優先，不覆蓋】
 *
 * 兩者互斥（`pr_advance_category_chk`，migration_202）。已經勾了暫支款
 * 還硬塞代墊的話，資料庫會 raise 一句看不懂的訊息，
 * 而使用者只是換了個事業體。所以有暫支款時回 null —— 那顆勾本來也是灰的。
 */
export function autoLend(
  book: string | null | undefined,
  cur: { advance_category?: string | null } | null | undefined,
): string | null {
  if (cur?.advance_category) return null;
  return lendFor(book, true);
}
