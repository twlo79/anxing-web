        ...
        <>
          ...
          <div className="filter-bar rounded-xl glass p-4 mb-4 flex flex-wrap items-end gap-3 text-sm">
            ...
            <div className="ml-auto flex items-end gap-3">
              <div className="text-xs text-gray-400 pb-1.5">共 {filtered.length} 筆</div>
              <button onClick={() => setEdit({ status: 'watching' })}
                className="rounded-lg bg-mor-slate text-white px-4 py-1.5 font-medium hover:bg-mor-slatedark">
                ＋ 新增標案
              </button>
            </div>
          </div>
                      ...
                      return (
                        <tr key={r.id} className="border-b border-mor-line/60 last:border-0 hover:bg-white/45">
                          ...
                          <td className="px-3 py-2.5 text-right whitespace-nowrap">
                            ...
                            <button onClick={() => setEdit(r)} className="text-mor-slate underline">檢視</button>
                          </td>
                        </tr>
                      );
                    })}
        ...
        <div className="modal-backdrop" onClick={() => setEdit(null)}>
          <div className="modal-panel max-w-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body space-y-3 text-sm">
              ...
              */}
              {edit.id
                ? <Receipts kind="td" parentId={edit.id} label="文件與照片" />
                : <div className="text-xs text-gray-400">先存檔之後才能上傳文件與照片。</div>}
            </div>
